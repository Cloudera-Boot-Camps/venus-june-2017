...
## Constants
APP_NAME = "pyspark-gravitational-wave-batch"
JDBC = "jdbc"
...
def load(sqlc,hc):
    gal_df = sqlc.read.format(JDBC).options(url=ORA_URL,driver=ORA_DRIVER,dbtable=tables[0],user=USER,password=PASSWORD).load()
    det_df = sqlc.read.format(JDBC).options(url=ORA_URL,driver=ORA_DRIVER,dbtable=tables[1],user=USER,password=PASSWORD).load()
    ast_df = sqlc.read.format(JDBC).options(url=ORA_URL,driver=ORA_DRIVER,dbtable=tables[2],user=USER,password=PASSWORD).load()
